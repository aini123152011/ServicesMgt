...
describe('ConfirmDialog', () => {
  it('renders title, description, and default buttons', async () => {
    ...
    await expect
      ...
      .toBeInTheDocument()
    await expect
      .element(getByRole('button', { name: '确定' }))
      .toBeInTheDocument()
  })
  ...
  it('disables confirm when disabled is true', async () => {
    ...
    const confirm = getByRole('button', { name: '确定' })
  ...
  it('when isLoading is true, disables cancel and confirm', async () => {
    ...
    await expect.element(getByRole('button', { name: '确定' })).toBeDisabled()
  })
  ...
  it('renders confirm as submit button linked to desc form when `form` is set', async () => {
    const { getByRole } = await render(
      <ConfirmDialog
        ...
        desc={
          <form id='service-delete-form' className='space-y-4'>
    ...
    await expect
      .element(deleteBtn)
      .toHaveAttribute('form', 'service-delete-form')
  })
