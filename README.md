...
> 改不了端口，去掉就失去测试意义。
>
> DHCP 是唯一的例外：它靠二层广播工作，Docker 的端口映射转发不了广播，所以只发布 DNS 的
> 18113；要让现场 BMC 真正从这个服务取址，需把它改挂 macvlan——在平台
> 「设置 → 关于 → 二层夹具绑定」里启用/切换即可（见[部署指南](docs/deployment.md)）。
...
- [容器运行时踩坑清单](docs/container-runtime-guidelines.md) —— 12 类「配置写对了但服务没按配置工作」的坑与修法
